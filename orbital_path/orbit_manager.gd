...
func add_structure(structure: Structure, orbit_idx: int) -> void:
	if structure.is_orbital:
		var radius: float      = orbit_radii[orbit_idx]
		var direction: Vector2 = (structure.position - orbit_center).normalized()
...
func add_orbit(radius: float) -> void:
	...
	var last_multiplier: float = orbit_speed_multipliers[-1]
...
func _process(delta):
	...
	for structure in structures:
		if structure.is_orbital:
			...
			var multiplier: float = orbit_speed_multipliers[orbit_idx]
...
func update_positions() -> void:
	for i in range(structures.size()):
		...
		if structure.is_orbital:
			...
			var angle: float = (float(i) / structures.size()) * TAU
			var pos: Vector2 = orbit_center + Vector2(cos(angle), sin(angle)) * orbit_radii[i]
...
func _draw():
	...
	var line_width: float = base_width * zoom_scale * 2.0

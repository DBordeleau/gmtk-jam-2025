...
func _create_upgrade_cards():
	...
	for i in range(upgrades.size()):
		...
		var card: Control    = _create_upgrade_card(upgrade, i)
...
func _create_upgrade_card(upgrade: Upgrade, index: int) -> Control:
	...
	# Add hover animation
	_setup_card_hover_animation(container, card, style_box)

	return container


func _setup_card_hover_animation(container: Control, card: Panel, original_style: StyleBoxFlat):
	var hover_style: Resource = original_style.duplicate()
	...
	var button: Node = container.get_child(1)  # The invisible button overlay

	button.mouse_entered.connect(func():
		card.add_theme_stylebox_override("panel", hover_style)
		var tween = create_tween()
		tween.tween_property(container, "scale", Vector2(1.05, 1.05), 0.2).set_trans(Tween.TRANS_QUART).set_ease(Tween.EASE_OUT)
	)
...
func _animate_in():
	...
	for i in range(card_container.get_child_count()):
		var card: Node        = card_container.get_child(i)
